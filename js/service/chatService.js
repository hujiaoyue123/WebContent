/** 用户*/
var chatServiceDef = function($rootScope,toastr,$$user,$$data,$timeout,$$util,$ionicPopup,$state,$$data
		
		// 以下是移动端注入
		,$$mobile){
		var chatService = {
	        msgData:[],
			isWeb : UPF.is("WEB"),
	        https : $rootScope.CONFIG.chat.https,
	        url : $rootScope.CONFIG.chat.xmppURL,
	        apiUrl : '',
	        appKey : $rootScope.CONFIG.chat.appKey,
	        loginOK : false,
	        conflict : false,
	        conn:null,
	        listHistory:false,
	        initOK:false,
	        initParam:{
	            https : config.chat.https,
	            url: config.chat.xmppURL,
	            apiUrl: config.chat.apiUrl,
	            //当连接成功时的回调方法
	            onOpened : function() {
	            	chatService.opened();
	            },
	            //当连接关闭时的回调方法
	            onClosed : function() {
	            	chatService.close();
	            },
	            //收到文本消息时的回调方法
	            onTextMessage : function(msg) {
	            	chatService.handleTextMsg(msg);
	            },
	            //收到图片消息时的回调方法
	            onPictureMessage : function(msg) {
	            	chatService.handlePictureMsg(msg);
	            },
	            //异常时的回调方法
	            onError : function(message) {
	            	chatService.error(message);
	            },
	            //收到音频消息的回调方法
	            onAudioMessage : function(message) {
	            	chatService.handleAudioMsg(message);
	            },
/*		            //收到表情消息时的回调方法
	            onEmotionMessage : function(message) {
	                handleEmotion(message);
	            },
	            //收到位置消息的回调方法
	            onLocationMessage : function(message) {
	                handleLocationMessage(message);
	            },
	            //收到文件消息的回调方法
	            onFileMessage : function(message) {
	                handleFileMessage(message);
	            },
	            //收到视频消息的回调方法
	            onVideoMessage : function(message) {
	                handleVideoMessage(message);
	            },
	            //收到联系人订阅请求的回调方法
	            onPresence : function(message) {
	                handlePresence(message);
	            },
	            //收到联系人信息的回调方法
	            onRoster : function(message) {
	                handleRoster(message);
	            },
	            //收到群组邀请时的回调方法
	            onInviteMessage : function(message) {
	                handleInviteMessage(message);
	            },
	            */
	        },
	        init:function(){
	        	this.msgData.unread = 0;
	        	$rootScope.msgUnread = 0;
	        	if ( true ){  //this.isWeb
	        		if (!this.conn){
	        			this.initConn();
	        		}
					if (!this.loginOK){
						if ($rootScope.USER){
							this.login({
			                    user : $rootScope.USER.id,
			                    pwd : $rootScope.USER.chatpwd,
			                    username: $rootScope.USER.username
							});
						}
					}
	        	}
	        },
	        refreshMsgs:function(param){
	        	var me = this;
	        	if(param.startMsgId){
	        		if(!this.isWeb){
	        			this.conn.chat_getMessage(param,function(msgs){
							if(typeof(msgs) == "string"){
								msgs = JSON.parse(msgs);
							}
	    					for (var i=msgs.length-1;i>=0;i--){
	    						if (me.updateMsgType(msgs[i],true)){
	    							me.unshiftMsg(msgs[i], i);
	    						}
	    					}
	    					if(msgs.length>0){
	    						$rootScope.Map.put(param.id,msgs[0].id);
	    					}
	        			});
        			}
	        	}
	        },
	        unshiftMsg:function(msg,i){
	        	var selfMsg = false;
        		var msgtime = (new Date()).getTime();
        		try{
        			msgtime = msg.ext.ext.time;
        		}catch(e){
        			
        		}
        		var md = this.msgData;
        		var peerId;
        		if (msg.from == $rootScope.USER.id){
        			msg.peerId = peerId = msg.to; 
        			selfMsg = true;
        		}else if (msg.type == "groupchat"){
        			msg.peerId = peerId = msg.to;
        		}else{
        			msg.peerId = peerId = msg.from;
        		}
        		var x = this.indexOf(peerId);
        		var conv;
        		if(x>=0){
        			conv = md.splice(x,1)[0];
        			if(i==0){
        				var ct = msgtime;
    					conv.moment = ct;
    					msg.moment = conv.moment;
        			}
        		}else{
        			conv={
        				id:peerId,
        				peerId:peerId,
        				type:msg.type,
        				msgs:[],
        				unread: 0,
        				moment:msgtime
        			};
        			msg.moment = msgtime;
        		}
        		
    			conv.msgs.unshift(msg);
    			md.unshift(conv);
        		if(x<0){
        			var self = this;
        			$$user.contacts.getUser(conv, function(r){
        				if(r){
        					conv.peer = r;
	        				conv.groupId = r.groupId;
	            			$rootScope.$broadcast("chatMsg",{msg:'gotMsg'});
        				}else{
        					conv.peer = {name : '无效用户'};
        				}
	            	});
        		}
        		if(msg.from != $rootScope.USER.id){
        			var self = this;
	        		$$user.contacts.getUser({id:msg.from,type:'chat'},function(r){
	        			msg.user = r;
	        			if(msg.type == "groupchat" && (conv.msgs.indexOf(msg) == (conv.msgs.length-1))){
	        				conv.text = msg.user.name+':'+msg.data;
	        			}
	        			$rootScope.$broadcast("chatMsg",{msg:'refushUser'});
	        			
	        		})
        		}
	        },
        	getMsgs:function(param){
        		var id = param.id;
        		// 如果有id，只取一个peer的消息，没有的话，取全部会话
        		if(id){
		    		var x = this.indexOf(id);
		    		var u = this.msgData[x];
		    		if(u){
		    			if(!u.historyed){
		        			if(!this.isWeb){
		        				u.msgs = [];
		        				u.moment = 0;
		        			}
		    				this.getHistory(param);
		    				u.historyed = true;
		    			}else{
		    				u.at = false;
		    			}
			    		this.msgData.unread = (this.msgData.unread - u.unread>=0) ? (this.msgData.unread - u.unread):0;
		    			$rootScope.msgUnread = this.msgData.unread;
		    			try{
		    				$rootScope.$digest();
		    			}catch(e){}
			    		u.unread = 0;
			    		if(!this.isWeb){
			    			this.conn.chat_markRead({id:id},null,function(msg){
			    			})
			    		}else{
			    			localStorage['msgData'+$rootScope.USER.id] = angular.toJson(this.msgData);
			    		}
		    		}
		    		param.callback(u);
        		}else{
        			if(!this.listHistory){
        				this.listHistory = true;
        				this.getHistory(param);
        			}
       				param.callback(this.msgData);
       				//统计未读
       				this.countUnread();
        		}
        	},
        	getHistory:function(param){
        		var id = param.id;
				var me = this;
        		if (id){
        			if(!this.isWeb){
	        			this.conn.chat_getMessage(param,function(msgs){
							if(typeof(msgs) == "string"){
								msgs = JSON.parse(msgs);
							}
	    					for (var i=0;i<msgs.length;i++){
	    						if (me.updateMsgType(msgs[i],true)){
	    							me.addMsg(msgs[i], true);
	    						}
	    					}
	    					if(msgs.length>0){
	    						$rootScope.Map.put(id,msgs[0].id);
	    					}
	        			});
        			}
        		}else{
        			if(!this.isWeb){
        				this.conn.chat_loadConversationsWithRecentChat({},function(msgs){
							if(typeof(msgs) == "string"){
								msgs = JSON.parse(msgs);
							}
							var t = 0;
        					for (var i=0;i<msgs.length;i++){
        						//me.msgData[i].unread =  msgs[i].unReadNo;
        						t += msgs[i].unReadNo;
        						if(me.updateMsgType(msgs[i],true)){
        							me.addMsg(msgs[i],true,true);
        						}
        					}
//        					if(!UPF.is("android")){
    			    			$rootScope.msgUnread = t;
//        					}
        					
            				param.callback(me.msgData);
        				});
//        				if(UPF.is("android")){
//			    			this.conn.chat_unReadNo({},function(r){
//		        				$rootScope.msgUnread = parseInt(r.msgUnread);
//        	    				$rootScope.$digest();
//		        			},function(){});
//			    		}
        			}else{
        				//已在opened中执行（但是考虑到opened有延迟）
        				if(this.msgData.length == 0){
        					if(localStorage['msgData'+$rootScope.USER.id]){
        						try{
        							this.msgData = localStorage['msgData'+$rootScope.USER.id]? JSON.parse(localStorage['msgData'+$rootScope.USER.id]):[];
        							//统计未读
        							this.countUnread();
        						}catch(e){
        							localStorage['msgData'+$rootScope.USER.id] = [];
        							this.msgData = [];
        						}
        					}
        				}
        			}
        		}
        	},
        	saveWebHistory:function(){
				localStorage['msgData'+$rootScope.USER.id] = angular.toJson(this.msgData);
        	},
        	/** 添加最新消息
        	 * @param msg {
        	 * 		body:
        	 * 		data:
        	 * 		ext: {
        	 * 			ext: {
        	 * 				time:
        	 * 			}
        	 * 		}
        	 * 		from:
        	 * 		id: 
        	 * 		msgType:
        	 * 		to:
        	 * 		type:
        	 * 	}
        	 */
        	addMsg:function(msg,history,list){
        		var selfMsg = false;
        		//收到消息的时间
        		var msgtime = (new Date()).getTime();
        		try{
        			msgtime = msg.ext.ext.time;
        		}catch(e){
        			
        		}
        		var md = this.msgData;
        		var peerId;
        		if (msg.from == $rootScope.USER.id){ //发消息人是当前人
        			msg.peerId = peerId = msg.to; 
        			selfMsg = true;
        		}else if (msg.type == "groupchat"){ //群组聊天
        			msg.peerId = peerId = msg.to;
        		}else{ //其它: 收到别人的消息
        			msg.peerId = peerId = msg.from;
        		}
        		var x = this.indexOf(peerId);
        		var conv;
        		//本地消息含有此类消息记录
        		if(x>=0){ //更新上次回话时间
        			conv = md.splice(x,1)[0];
        			if(!conv.moment || ((msgtime - conv.moment) > 300000)){
        				var ct = msgtime;
    					conv.moment = ct;
    					msg.moment = conv.moment;
        			}
        		}else{ //初始化一个新的回话
        			conv={
        				id:peerId,
        				peerId:peerId,
        				type:msg.type,
        				msgs:[],
        				unread: 0,
        				moment:msgtime
        			};
        			msg.moment = msgtime;
        		}
        		//会话装载最新消息
    			conv.msgs.push(msg);
    			//会话栏-最新消息显示的文本
    			if (msg.msgType == 'zwfile'){ //掌文文件
    				conv.text = '[文件]'+msg.ext.ext.filename;
    			}
    			//审核文件消息
    			else if(msg.msgType == 'auditfile'){
    				conv.text = msg.ext.ext.title;
    			}
    			else if(msg.msgType == 'voice'){ //语音
    				conv.text = '[语音]';
    			}else{ //其它
    				conv.text = msg.data;
    			}
    			if(!history && !selfMsg && ($rootScope.currentChatId != peerId)){ //没有会话历史 && 不是自己发的消息 && 当前打开的会话不是该新消息的会话
    				conv.unread = conv.unread?conv.unread+1:1; //该会话未读+1
					md.unread = md.unread?md.unread+1:1; //该会话集未读+1
    				// 判断是否有@
    				if(msg.ext.ext.at){
    					var ata = msg.ext.ext.at.split(',');
    					for(var atau in ata){
    						if (ata[atau] == $rootScope.USER.username){
    							conv.at = true;
    							break;
    						}
    					}
    				}
    			}else if(list){
    				conv.unread = msg.unReadNo || 0;
    				conv.at = msg.at;
    			}
    			//新消息的会话列置顶
    			md.unshift(conv);
    			if(!list && !UPF.is("android")){ // list为空 && 不是android
    				$rootScope.msgUnread = md.unread; //标记总未读消息条数
    			}
    			//当前会话列表无此新消息会话
        		if(x<0){
        			var self = this;
        			$$user.contacts.getUser(conv, function(r){
        				if(r){
        					conv.peer = r;
	        				conv.groupId = r.groupId;
	        				if(self.isWeb){
	        					localStorage['msgData'+$rootScope.USER.id] = angular.toJson(md);
	        				}
	            			$rootScope.$broadcast("chatMsg",{msg:'gotMsg'});
        				}else{
        					conv.peer = {name : '无效用户'};
        				}
	            	});
        		}
        		//新消息发送人不是当前人
        		if(msg.from != $rootScope.USER.id){
        			var self = this;
	        		$$user.contacts.getUser({id:msg.from,type:'chat'},function(r){
	        			msg.user = r;
	        			if(msg.type == "groupchat" && (conv.msgs.indexOf(msg) == (conv.msgs.length-1))){
	        				conv.text = msg.user.name+':'+msg.data;
	        			}
	        			if(self.isWeb){
	        				localStorage['msgData'+$rootScope.USER.id] = angular.toJson(md);
	        			}
	        			$rootScope.$broadcast("chatMsg",{msg:'refushUser'});
	        			
	        		})
        		}
        		if (!history && !selfMsg){
        			$rootScope.$broadcast("chatMsg",{msg:'gotMsg'});
        			try{
        				$rootScope.$digest();
        			}
        			catch(e){
        				
        			}
        		}
        		if (this.isWeb){
        			localStorage['msgData'+$rootScope.USER.id] = angular.toJson(md);
        		}
        	},
        	notify:function(peerId){
        		if (!$rootScope.silent && $rootScope.currentChatId != peerId){
        			if (this.isWeb){
            		document.getElementById("notify").play();
        			}else{
        				$$mobile.audio.play();
        			}
        		}
        	},
        	updateMsgType:function(msg, history){
        		var data = msg.data;
				if(!msg.data){
					msg.data= '　';
				}
        		if (data.slice(0,8) == '[zwfile]'){
        			msg.msgType = 'zwfile';
        			msg.data = '[文件]'+msg.ext.ext.filename;
    				if (msg.ext.ext.filename && !msg.ext.ext.icon){
    					var fn = msg.ext.ext.filename;
    					var fileext = fn.substr(fn.lastIndexOf('.')+1);
    					var file = {fileext:fileext,id:msg.ext.ext.zwfileid};
    					$$data.type(file);
    					$$data.icon(file);
    					msg.ext.ext.icon = file.src;
    				}
        		}
        		//审核文件
        		else if(data == '[auditfile]'){
        			msg.msgType = 'auditfile';
        			if(msg.ext.ext.isaudit == 0){
        				msg.data = msg.ext.ext.title;
        			}else{
        				msg.data = msg.ext.ext.title;
        			}
        		}
        		else if (data.slice(0,5) == '[url]'){
    				msg.msgType = 'url';
    				msg.data = '[链接]'+msg.ext.ext.title;
        		}else if(data.slice(0,5) == '[sys]'){
        			if(data.slice(5,14) == 'newInvite'){
        				if(!history){
	        				$rootScope.newInvite = $rootScope.newInvite? parseInt($rootScope.newInvite)+1:1;
	        				localStorage['newInviteNo'+$rootScope.USER.id] = $rootScope.newInvite;
	        				$rootScope.$digest();
	            			this.notify(1);
	        			}
        			}else if(data.slice(5,17) == 'acceptInvite' || data.slice(5,17) == 'deleteFriend'){
        				$rootScope.$broadcast('zw.refreshContact');
        			}else if(data.slice(5,15) == 'changeName'){
        				var newGroupName = data.slice(16);
        				this.updateGroupName(msg.to, newGroupName);
        			}else if(data.slice(5,12) == 'newFile'){
        				this.notify(1);
            			$rootScope.$broadcast("fileMsg",{msg:'gotFile'});
        			}
       				return false;
        		}else if(msg.msgType == 'pic'){
        		}else if(msg.msgType == 'voice'){
        		}else{
        			msg.body = this.parseTextMessage(msg.data).body;
        			msg.msgType = 'txt';
        		}
        		return true;
        	},
        	/**
        	 * 修改群名称
        	 */
        	updateGroupName:function(hxid, newName){
        		var i = this.indexOf(hxid);
        		//msgData有此条消息
        		if (i>=0){
    				this.msgData[i].name = newName;
    				if(this.msgData[i].peer){
    					this.msgData[i].peer.name = newName;
    				}
        		}
        		var p = $$user.contacts.indexOf(hxid);
        		if (p && p.name){
    				p.name = newName;
        		}
        		$rootScope.$broadcast("chatMsg",{msg:'changeName', newName:newName});
        	},
            handleEmotion:function(msg){
            	if(true){
            		this.addMsg(msg);
        			this.notify(msg.peerId);
            	}
            },
        	handleTextMsg:function(msg){
        		try{
        			if (this.updateMsgType(msg)){
            			this.addMsg(msg);
            			if(UPF.is("android")){
            				var self = this;
        	    			this.conn.chat_unReadNo({},function(r){
        	    				self.msgData.unread = parseInt(r.msgUnread);
                				$rootScope.msgUnread = parseInt(r.msgUnread);
        	    				$rootScope.$digest();
                			},function(){});
        	    		}
            			this.notify(msg.peerId);
            		}
        		}catch(e){
        			console.log(e);
        		}
        		
        	},
        	handlePictureMsg:function(message){
        		try{
        			message.msgType = 'pic';
            		if(this.isWeb){
            			message.data = '[图片]';
        	            message.image = true;
        	            message.filePath = message.url;
        	            this.addMsg(message);
            		}else{
                		this.addMsg(message);
                		if(UPF.is("android")){
            				var self = this;
        	    			this.conn.chat_unReadNo({},function(r){
        	    				self.msgData.unread = parseInt(r.msgUnread);
                				$rootScope.msgUnread = parseInt(r.msgUnread);
        	    				$rootScope.$digest();
                			},function(){});
        	    		}
        	    		
            		}
            		this.notify(message.peerId);
        		}catch(e){
        			console.log(e);
        		}
        		
        	},
        	handleAudioMsg:function(message){
        		try{
        			message.msgType = 'voice';
            		if(this.isWeb){
            			message.data = '[语音]';
        	            this.addMsg(message);
            		}else{
                		this.addMsg(message);
                		if(UPF.is("android")){
            				var self = this;
        	    			this.conn.chat_unReadNo({},function(r){
        	    				self.msgData.unread = parseInt(r.msgUnread);
                				$rootScope.msgUnread = parseInt(r.msgUnread);
        	    				$rootScope.$digest();
                			},function(){});
        	    		}
        	    		
            		}
            		this.notify(message.peerId);
        		}catch(e){
        			console.log(e);
        		}
        	},
        	/**
        	 * 语音已读
        	 * @param convId
        	 * @param msgId
        	 */
        	setAudioListened: function(param,successCallback){
        		this.conn.chat_isListened(param,successCallback);
        	},
        	deleteCoversation:function(id){
        		if (id){
        			var i = this.indexOf(id);
        			if (i>-1){
        				var c = this.msgData.splice(i, 1)[0];
		    			$rootScope.msgUnread = this.msgData.unread = $rootScope.msgUnread - c.unread;
        				localStorage['msgData'+$rootScope.USER.id] = angular.toJson(this.msgData);
        				if(!this.isWeb){
        					this.conn.chat_deleteConversation({username:id});
        				}
        			}
        		}
        	},
        	/**
        	 * 判断this.msgData是否含有该uid的元素
        	 */
        	indexOf:function(uid){
        		var md = this.msgData;
        		for (var i=0; i<md.length; i++){
        			var u = md[i];
        			if (u.id == uid){
        				return i;
        			}
        		}
        		return -1;
        	},
			login:function(p){
        		if(!$rootScope.CONFIG.internet) {return;}
				if(this.isWeb){
					if(!this.opening){
						this.opening = true;
						this.user = p;
						this.loginCount = 1;
							this.conn.open({
				                apiUrl : this.apiUrl,
				                user : p.user,
				                pwd : p.pwd,
				                appKey : this.appKey
				            });
					}
				}else{
					if(this.initOK){
						this.conn.chat_login({
							username:p.user,
							password:p.pwd,
							nickname:p.username
						});
					}
				}
			},
			/**
			 * 统计未读
			 */
			countUnread: function(){
				var unread = 0;
				for(var i=0;i<this.msgData.length;i++){
					if(this.msgData[i].unread){
						unread += parseInt(this.msgData[i].unread);
					}
				}
				this.msgData.unread = unread;
				$rootScope.msgUnread = unread;
			},
        	opened:function(){
        		this.opening = false;
				$rootScope.newInvite = parseInt(localStorage['newInviteNo'+$rootScope.USER.id]);
        		if(this.isWeb){
	                if (this.conn.isOpened()) {
	                	this.loginOK = true;
	                	this.conn.heartBeat(this.conn);
	            		$rootScope.$broadcast("chatMsg",{msg:'loginOK'});
	                }
	                this.conn.setPresence();
	                //消息未读
	                this.msgData = localStorage['msgData'+$rootScope.USER.id]? JSON.parse(localStorage['msgData'+$rootScope.USER.id]):[];
	                this.countUnread();
        		}else{
        			this.loginOK = true;
        			var self = this;
        			this.conn.chat_unReadNo({},function(r){
        				$rootScope.msgUnread = parseInt(r.msgUnread);
        				self.msgData.unread = $rootScope.msgUnread;
        				/* 未读与新邀请提醒完全不搭嘎..
        				if (parseInt(r.sysUnread) > 0){
        					$rootScope.newInvite = parseInt($rootScope.newInvite) + parseInt(r.sysUnread);
        				}
        				*/
            			$rootScope.$broadcast("chatMsg",{msg:'gotMsg'});
        			},function(){});
        			
            		$rootScope.$broadcast("chatMsg",{msg:'loginOK'});
        		}
        	},
        	close:function(){
        		this.opening = false;
            	this.loginOK = false;
            	if(this.isWeb && this.conn.heartBeatID) {
            		this.conn.stopHeartBeat(this.conn);
            		this.conn.clear();
            	}
        	},
        	error: function(e){
        		if (e.msg == 'conflict'){
        			this.conflict = true;
            		$rootScope.$broadcast("chatMsg",{msg:'conflict'});
            		/** 如果在别的地方登陆，就强制退出 */
            		if($rootScope.conflictPopup){
            			$rootScope.conflictPopup.close();
            		}
            		$rootScope.conflictPopup = $ionicPopup.show({
            			title: "提醒",
            			subTitle: "您的账号在别处登陆，是否退出？",
            			buttons: [
            				{
            					text: "退出登录",
            					type: 'button-positive',
            					onTap: function(e) {
            						$$user.logout();
            						if (UPF.is("web")){
            							$state.go("loginPc");
            						}else{
            							$state.go("loginMobile");
            						}
            					}
            				},
            				{
            					text: "重新登录",
            					type: 'button-positive',
            					onTap: function(e) {
            						chatService.login({
        			                    user : $rootScope.USER.id,
        			                    pwd : $rootScope.USER.chatpwd,
        			                    username: $rootScope.USER.username
        							});
            						$rootScope.conflictPopup.close();
            					}
            				}
            			]
            		});
        		}else if(this.isWeb && e.data){
	            	var param = {
	            		method:'POST',
	            		action:"chatLoginFail",
	            		fm:e.data.error_description
	            	};
	            	$$user.callService(param,function(){
	            		if (!chatService.loginOK && chatService.loginCount<4 && chatService.loginCount>0){
	            			chatService.loginCount++;
	            			$timeout(chatService.login(chatService.user),2000);
	            		}
	            	});
        		}
        	},
        	groupError:function(group, fn){
        		var param = {
        			method:'POST',
        			action:'updateChatGroup',
        			id:group.groupid,
        			hxid:group.hxid
        		};
        		$$user.callService(param,function(r){
        			if (r.result == 1){
        				fn(r.hxid);
        			}else{
        				toastr.error("群组聊天功能错误，请联系客服");
        			}
        		});
        	},
		    initConn:function(){
		    	if (this.isWeb){
		    		this.conn = new Easemob.im.Connection();
			    }else{
			    	this.conn = cordova.plugins.EasemobChat;
			    }
		    	this.conn.init(this.initParam);
		    	this.initOK = true;
			},
			sendMsg:function(param,successCallback){
				param.ext = param.ext || {ext:{}};
				param.ext.ext.time = (new Date()).getTime();
				this.conn.sendTextMessage(param,successCallback);
			},
			sendPicture:function(param){
				param.ext={ext:{time:(new Date()).getTime()}};
				this.conn.sendPicture(param);
			},
			addPeer:function(u){
				if(this.indexOf(u.id)<0){
					this.msgData.unshift(u);
				}
			},
			clearVoiceCache:function(){
				this.conn.chat_clearCache();
			},
			emotionPicData : [
		        {id:"[微笑]",data:"data:image/png;base64,"},
		        {id:"[大笑]",data:"data:image/png;base64,"},
		        {id:"[媚眼]",data:"data:image/png;base64,"},
		        {id:"[惊讶]",data:"data:image/png;base64,"},
		        {id:"[吐舌]",data:"data:image/png;base64,"},
		        {id:"[墨镜]",data:"data:image/png;base64,"},
		        {id:"[发怒]",data:"data:image/png;base64,"},
		        {id:"[撇嘴]",data:"data:image/png;base64,"},
		        {id:"[害羞]",data:"data:image/png;base64,"},
		        {id:"[难过]",data:"data:image/png;base64,"},
		        {id:"[大哭]",data:"data:image/png;base64,"},
		        {id:"[发呆]",data:"data:image/png;base64,"},
		        {id:"[天使]",data:"data:image/png;base64,"},
		        {id:"[生气]",data:"data:image/png;base64,"},
		        {id:"[眼镜]",data:"data:image/png;base64,"},
		        {id:"[呕吐]",data:"data:image/png;base64,"},
		        {id:"[小丑]",data:"data:image/png;base64,"},
		        {id:"[郁闷]",data:"data:image/png;base64,"},
		        {id:"[放空]",data:"data:image/png;base64,"},
		        {id:"[闭嘴]",data:"data:image/png;base64,"},
		        {id:"[争吵]",data:"data:image/png;base64,"},
		        {id:"[白眼]",data:"data:image/png;base64,"},
		        {id:"[啥]",data:"data:image/png;base64,"},
		        {id:"[爱]",data:"data:image/png;base64,"},
		        {id:"[心碎]",data:"data:image/png;base64,"},
		        {id:"[月亮]",data:"data:image/png;base64,"},
		        {id:"[星星]",data:"data:image/png;base64,"},
		        {id:"[太阳]",data:"data:image/png;base64,"},
		        {id:"[彩虹]",data:"data:image/png;base64,"},
		        {id:"[色]",data:"data:image/png;base64,"},
		        {id:"[嘟嘴]",data:"data:image/png;base64,"},
		        {id:"[亲亲]",data:"data:image/png;base64,"},
		        {id:"[玫瑰]",data:"data:image/png;base64,"},
		        {id:"[凋谢]",data:"data:image/png;base64,"},
		        {id:"[顶]",data:"data:image/gif;base64,"}
			],
			parseTextMessage : function(message){
			    if(typeof(message) != 'string'){
			        return false;
			    }
			    var receiveMsg = message;
			    var emessage = [];
			    var expr = /\[[^[\]]{1,3}\]/mg;
			    var emotions = receiveMsg.match(expr);
			    if (!emotions || emotions.length < 1){
			        return message;
			    }
			    var isemotion = false;
			    for (var i = 0; i < emotions.length; i++) {
			        var tmsg = receiveMsg.substring(0,receiveMsg.indexOf(emotions[i]));
			        if (tmsg) {
			            emessage.push({
			                "type" : "txt",
			                "data" : tmsg
			            });
			        }
			        
			        var emotion = $$util.ARRAY.findById(emotions[i], this.emotionPicData);
			        if (emotion) {
			            isemotion = true;
			            emessage.push({
			                "type" : "emotion",
			                "data" : emotion
			            });
			        } else {
			            emessage.push({
			                "type" : "txt",
			                "data" : emotions[i]
			            });
			        }
			        var restMsgIndex = receiveMsg.indexOf(emotions[i]) + emotions[i].length;
			        receiveMsg = receiveMsg.substring(restMsgIndex);
			    }
			    if (receiveMsg) {
			        emessage.push({
			            "type" : "txt",
			            "data" : receiveMsg
			        });
			    }
			    if(isemotion){
			        return {"isemotion":isemotion,"body":emessage};
			    }
			    return {"isemotion":false,"body":[{"type" : "txt","data":message}]};
			},
			/**
			 * 判断图片有更新，则更新会话和消息的头像
			 * @param user 要检测和更新的用户
			 */
			updateUserAvatar: function(user){
				var msgData = this.msgData;
				for(var i=0;i<msgData.length;i++){
					//最新头像路径
					var avatar = $$user.getUserPhoto(user.id,user.updatetime);
					var conv = msgData[i]; //会话
					//检测并更新会话体头像
					var peer = conv.peer;
					if(peer && peer.id == user.id && peer.updatetime != user.updatetime){
						conv.peer.avatar = avatar;
						conv.peer.updatetime = user.updatetime;
					}
					//检测并更新消息体头像
					var msgs = conv.msgs;
					for(var k=0;k<msgs.length;k++){
						var u = msgs[k].user;
						if(u && u.id == user.id && u.updatetime != user.updatetime){
							msgs[k].user.avatar = avatar;
							msgs[k].user.updatetime = user.updatetime;
						}
					}
				}
			}
		};
		$$user.$$chat = chatService;
		return chatService;
	};
	
if(UPF.is("WEB")){
	chatServiceDef.$inject = ["$rootScope","toastr","$$user","$$data","$timeout","$$util","$ionicPopup","$state","$$data"];
}
angular.module("app").factory("$$chat",chatServiceDef);	